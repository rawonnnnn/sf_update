...
class iOSMessageHandler(
    ...
    override fun userContentController(
        ...
        if (body.contains("\"playNextEpisode\"")) {
            onPlayNext()
        } else if (body.contains("\"updateProgress\"")) {
            val currentRegex = """currentTime":\s*([0-9.]+)""".toRegex()
            val durationRegex = """duration":\s*([0-9.]+)""".toRegex()
...
actual fun WatchPlayer(
    ...
    val lastLoaded = remember { mutableListOf<String>() }
    ...
    UIKitView(
        ...
        factory = {
            ...
            val webView = WKWebView(frame = CGRectMake(0.0, 0.0, 0.0, 0.0), configuration = configuration).apply {
        ...
        update = { webView ->
            ...
            val embed = episode.embedUrl
            val targetUrl = if (stream.isNotBlank()) stream else embed
            if (lastLoaded.firstOrNull() != targetUrl) {
                lastLoaded.clear()
                lastLoaded.add(targetUrl)
                if (stream.isNotBlank()) {
                    webView.loadHTMLString(htmlContent, baseURL = NSURL.URLWithString("https://sofaflix.top"))
